...
import {postRequest, patchRequest, deleteRequest} from "../../actions/Service";
import {
    ListView,
    View,
    Dimensions,
...
class AgentList extends Component {
    ...
    componentDidMount() {
        ...
        this.props.getAgentList(this.state.type, this.state.up_id, this.state.keyword, this.state.page_no);
    }

    componentWillReceiveProps(nextProps) {
        console.log('the state', nextProps);
        if (nextProps.data != null) {
            if (nextProps.requestType == 'agent_list') {
            ...
            }else if (nextProps.requestType == 'agent_up_resent_link') {
                Alert.alert(
                    'Success',
            ...
            }else if (nextProps.requestType == 'agent_up_delete_agent') {
                let rows = [];
    ...
    searchByKeyword() {
        this.setState({dataSource: [], page_no: 1, canLoadMoreContent: true, isRendering: true});
        this.props.getAgentList(this.state.type, this.state.up_id, this.state.keyword, 1);
    }
                ...
                if (this.state.type == 3) {
                    if (buttonIndex == 7) {
                        Alert.alert(
                            ...
                            [
                                {
                                    text: 'OK', onPress: () => {this.setState({deleting_agent_id: data.agent_id}); this.props.deleteAgent(data.agent_id);}
                                },
                                {
                                    text: 'Cancel', onPress: () => {}
                                }
                        ...
                        this.props.resendLink(data.agent_id, data.email, data.phone);
...
AgentList.navigationOptions = ({navigation}) => ({
    header: (
        <Header style={s.menuHeader}>
            ...
            <Body>
            <Title style={{width: 200, color: '#fff'}}>Agent List</Title>
            </Body>
            <Right>
...
const mapDispatchToProps = (dispatch) => {
    return {
        getAgentList: (type, up_id, keyword, page_no) => {
            dispatch(postRequest(Config.AGENT_LIST_URL, {type: type, up_id: up_id, keyword: keyword, page_no: page_no}, 'agent_list'));
        },
        resendLink: (agent_id, email, phone) => {
            dispatch(postRequest(Config.AGENT_RESEND_LINK_URL, {agent_id: agent_id, email: email, phone: phone}, 'agent_up_resent_link'));
        },
        deleteAgent: (agent_id) => {
            dispatch(deleteRequest(Config.PENDING_AGENT_LIST_URL + '/' + agent_id, 'agent_up_delete_agent'));
        }
    }
};
